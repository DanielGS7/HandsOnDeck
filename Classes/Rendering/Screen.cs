...
namespace HandsOnDeck2.Classes.Rendering
{
    public abstract class Screen : IScreen
    {
        ...
        {
            uiElements.Add(new Button(graphicsDevice, content, text, positionPercentage, sizePercentage, rotation, texture, onClick));
        }

        protected void AddCheckbox(Vector2 positionPercentage, Vector2 sizePercentage, string label)
        {
            uiElements.Add(new Checkbox(graphicsDevice, content, positionPercentage, sizePercentage, label));
        }

        protected void AddVolumeSlider(Vector2 positionPercentage, Vector2 sizePercentage)
        {
            uiElements.Add(new VolumeSlider(graphicsDevice, content, positionPercentage, sizePercentage));
        }
